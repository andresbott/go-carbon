...
func TestSessionManagement(t *testing.T) {
	...
	t.Run("access resource after login", func(t *testing.T) {
		svr, c := testServer(50*time.Millisecond, 200*time.Millisecond, 0)
		defer svr.Close()
	...
	t.Run("401 after session expired", func(t *testing.T) {
		svr, c := testServer(50*time.Millisecond, 500*time.Millisecond, 0)
		defer svr.Close()
		// perform login
		resp := doReq(c, svr.URL+"/login", t)
		want := http.StatusOK
	...
	t.Run("renew session", func(t *testing.T) {
		svr, c := testServer(50*time.Millisecond, 2000*time.Millisecond, 0)
		defer svr.Close()
		// perform login
		resp := doReq(c, svr.URL+"/login", t)
		want := http.StatusOK
	...
	t.Run("401 after max session duration", func(t *testing.T) {
		svr, c := testServer(500*time.Millisecond, 50*time.Millisecond, 0)
		defer svr.Close()
		// perform login
		resp := doReq(c, svr.URL+"/login", t)
		want := http.StatusOK
	...
	t.Run("401 forced session to not be updated", func(t *testing.T) {
		svr, c := testServer(50*time.Millisecond, 500*time.Millisecond, 5*time.Minute)
		defer svr.Close()
		// perform login
		resp := doReq(c, svr.URL+"/login", t)
		want := http.StatusOK
...
func TestAuthHandler(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			svr, client := testServer(50*time.Millisecond, 500*time.Millisecond, 5*time.Minute)
			defer svr.Close()
...
func testServer(SessionDur, MaxSessionDur, update time.Duration) (*httptest.Server, *http.Client) {
