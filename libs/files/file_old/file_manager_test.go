...
func TestNewFileManager_Nav(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			fm := files.Manager{ChRoot: tc.chdir}
...
func TestFileManager_Scan(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			fm.SetFilter(files.FilterCondition{})
			err := fm.Nav(tc.path)
			if err != nil {
				t.Fatalf("unexpected error: %s", err)
			}
...
func TestFileManager_Count(t *testing.T) {
	...
	tcs := []test{
		{
			...
			path:          "count",
			chroot:        tmpDir + "/root",
			filter:        files.FilterCondition{ShowHiddenFiles: true},
			expectedFiles: 8,
			expectedDirs:  5,
		},
		{
			...
			path:          "count",
			chroot:        tmpDir + "/root",
			filter:        files.FilterCondition{ShowHiddenFiles: false},
			expectedFiles: 5,
			expectedDirs:  3,
		},
		{
			...
			path:          "count",
			chroot:        tmpDir + "/root",
			filter:        files.FilterCondition{ShowHiddenFiles: false, ShowType: []files.FileType{files.AUDIO}},
			expectedFiles: 1,
			expectedDirs:  3,
		},
		{
			...
			path:          "count",
			chroot:        tmpDir + "/root",
			filter:        files.FilterCondition{ShowHiddenFiles: true, ShowExtension: []string{"jpg"}},
			expectedFiles: 7,
			expectedDirs:  5,
		},
	}
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			fm := files.Manager{ChRoot: tc.chroot}
...
func TestFileManager_e2e_Navigation(t *testing.T) {
	...
	fm := files.Manager{ChRoot: tmpDir + "/root"}
