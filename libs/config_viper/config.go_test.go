...
func TestConfig2(t *testing.T) {
	tcs := []struct {
		...
		},
		{
			...
			options: []Option{{Typ: EnvVarPrefix, Value: "TEST"}},
			envs: map[string]string{
				"TEST_ISDEVMODE":    "false",
				"TEST_GENERAL.PORT": "9090",
			},
		...
		},
		{
			...
			path:    TwelveFactor{},
			options: []Option{{Typ: EnvVarPrefix}},
			envs: map[string]string{
				"ISDEVMODE":    "false",
				"GENERAL.PORT": "9090",
			},
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			cfg, err := Load(tc.path, tc.options...)
			if err != nil {
				t.Fatal(err)
			}
...
func TestConfig(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			cfg, err := Read(tc.path, envPrefix)
			if err != nil {
				t.Fatal(err)
			}
...
func TestConfigErrors(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			_, err := Read(tc.path, tc.envPrefix)
			if err == nil {
