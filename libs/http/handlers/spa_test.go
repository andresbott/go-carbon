...
func _TestSpaHandler(t *testing.T) {
	tcs := []struct {
		name    string
		reqPath string
		subDir  string
		...
		{
			name:    "root on folder without index returns 404",
			reqPath: "/",
			subDir:  "testdata/ui/assets",
		...
		{
			name:    "file name as folder is 404",
			reqPath: "/assets/style.css/",
			subDir:  "testdata/ui",
			expect:  http.StatusNotFound,
		},
		...
		{
			name:    "dont allow folder content",
			reqPath: "/assets/",
			subDir:  "testdata/ui",
			expect:  http.StatusNotFound,
		},
